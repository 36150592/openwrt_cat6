                ...
                if (type == USB_DT_INTERFACE) {
                    ...
                    if (interface->bNumEndpoints == 2 &&
                        ...
                        if (!fstat(fd, &st)) {
                             ...
                             */
                            {
                            	//~ # cat /sys/bus/usb/devices/1-1.4/dev
                            	//189:12
                            	//获取设备号信息
                            	int major_numer,minor_number;
                            	if( !cmd_cat( "/sys/bus/usb/devices/1-1.4/dev",pathbuf,sizeof( pathbuf ) )
                            		|| !cmd_cat( "/sys/bus/usb/devices/1-1.1/dev",pathbuf,sizeof( pathbuf ) )
                            		)
                            	{
                            		char* tmp_ptr=strstr( pathbuf,":" );
                            		major_numer=atoi( pathbuf );
                            		if( tmp_ptr )
                            		{
                            			minor_number=atoi( tmp_ptr+1 );
                                        snprintf(pathbuf, sizeof(pathbuf), "/sys/dev/char/%d:%d",
                                        		major_numer, minor_number);
                            		}
                            		else
                            		{
                            			exit(-1);
