...
#endif
static const char* bm806_hardware_summary[]=
{
	"BM806U-C1",
	"BM806U-T1",
	"BM806U-E1",
...
static int get_network_mode_module(int network_mode_web,char* module_hardware)
{
	...
	const MCT *p=NULL;
	int len=0;
	for(i=0;i<ARRAY_MEMBER_COUNT(bm806_hardware_summary);i++)
	{
		if(!strncmp(bm806_hardware_summary[i],module_hardware,strlen(bm806_hardware_summary[i])))
			break;
	}
	switch(i)
	{
		...
		case 4:
			p=bm806c_mode_table;
			len=ARRAY_MEMBER_COUNT(bm806c_mode_table);
			break;
		default:
			p=bm806_mode_table;
			len=ARRAY_MEMBER_COUNT(bm806_mode_table);
			break;
...
void bm806_init(int*  Dial_proc_state)
{
	log_info("%s %d\n",__FUNCTION__,__LINE__);
	...
	switch(*Dial_proc_state)
	{
		...
		case Dial_State_initialized:
				memcpy(global_dial_vars.moduleType,"bm806",5);
...
void bm806_dial(int* Dial_proc_state )
{
...
void bm806_deamon(int* Dial_proc_state)
{
...
void bm806_report_handle(char* buffer)
{
...
void bm806_get_moduleinfo(MDI* p)
{
	int flag_cmd;
	int bytes_n=0;
	fd_set readfds;
