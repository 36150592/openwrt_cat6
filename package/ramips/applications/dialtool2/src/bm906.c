...
#endif
static const char* bm906_hardware_summary[]=
{
	"BM906"
};
...
static int get_network_mode_module(int network_mode_web,char* module_hardware)
{
	...
	const MCT *p=NULL;
	int len=0;
	for(i=0;i<ARRAY_MEMBER_COUNT(bm906_hardware_summary);i++)
	{
		if(!strncmp(bm906_hardware_summary[i],module_hardware,strlen(bm906_hardware_summary[i])))
			break;
	}
	switch(i)
	{
		...
		case 4:
			p=bm906c_mode_table;
			len=ARRAY_MEMBER_COUNT(bm906c_mode_table);
			break;
		default:
			p=bm906_mode_table;
			len=ARRAY_MEMBER_COUNT(bm906_mode_table);
			break;
...
void bm906_init(int*  Dial_proc_state)
{
	log_info("%s %d\n",__FUNCTION__,__LINE__);
	...
	switch(*Dial_proc_state)
	{
		...
		case Dial_State_initialized:
				memcpy(global_dial_vars.moduleType,"bm906",5);
...
void bm906_dial(int* Dial_proc_state )
{
...
void bm906_deamon(int* Dial_proc_state)
{
...
void bm906_report_handle(char* buffer)
{
...
void bm906_get_moduleinfo(MDI* p)
{
	int flag_cmd;
	int bytes_n=0;
	fd_set readfds;
