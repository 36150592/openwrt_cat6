...
#define VpMpiCmdWrapper1(deviceId,ecVal,mpiCmd,mpiCmdLen,dataBuffer) do{\
							tz_dbg(LOG_LEVEL_HAL, "VpMpiCmdWrapper cmd=0x%02x at %s %d", mpiCmd, __FILE__, __LINE__);\
							_VpMpiCmdWrapper(deviceId,ecVal,mpiCmd,mpiCmdLen,dataBuffer);\
							}while(0)   

#define VpMpiCmdWrapper(deviceId,ecVal,mpiCmd,mpiCmdLen,dataBuffer) do{\
							_VpMpiCmdWrapper(deviceId,ecVal,mpiCmd,mpiCmdLen,dataBuffer);\
							}while(0)   
...
#define VpSlacRegRead1(pDevCtx,pLineCtx,cmd,readLen,pReadBuf) do{\
							tz_dbg(LOG_LEVEL_HAL, "VpSlacRegRead cmd=0x%02x at %s %d", cmd, __FILE__, __LINE__);\
							_VpSlacRegRead(pDevCtx,pLineCtx,cmd,readLen,pReadBuf);\
							}while(0)     
#define VpSlacRegRead(pDevCtx,pLineCtx,cmd,readLen,pReadBuf) do{\
								_VpSlacRegRead(pDevCtx,pLineCtx,cmd,readLen,pReadBuf);\
								}while(0)  
