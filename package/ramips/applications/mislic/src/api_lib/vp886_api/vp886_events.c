...
char *get_local_time(void)
{
	...
	snprintf(localtimestr, 31, "%04d-%02d-%02d %02d:%02d:%02d",
		timenow->tm_year + 1900, timenow->tm_mon+1, timenow->tm_mday,
		timenow->tm_hour, timenow->tm_min, timenow->tm_sec);
	return localtimestr;
}

char *get_sys_usec_time(void)  
{  
    struct timeval    tv;  
    struct timezone tz;  
    ...
    sprintf(buff, "%d-%02d-%02d %02d:%02d:%02d.%ld", 
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    if (overflow != 0) {
        if (!pLineObj->dtmf.overflow) {
            ...
            //VP_WARNING(VpLineCtxType, pLineCtx, ("Vp886DtmfSampling: Overflow detected (%d) at %s", 
            //									overflow,get_sys_usec_time()));
    ...
    if (overflow == 0 && pLineObj->dtmf.overflow) {
        ...
        //VP_WARNING(VpLineCtxType, pLineCtx, ("Vp886DtmfSampling: Overflow cleared at %s",get_sys_usec_time()));
