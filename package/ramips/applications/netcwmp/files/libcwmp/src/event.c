...
static inform_event_t inform_event_table[] =
{
    ...
    {INFORM_XCMCC_BIND,                 		0,	CWMP_INFORM_EVENT_CODE_18, "inform:event_xcmcc_bind",                	"inform:command_xcmcc_bind"},
    {INFORM_XCMCC_MONITOR,                 		0,	CWMP_INFORM_EVENT_CODE_19, "inform:event_xcmcc_monitor",                "inform:command_xcmcc_monitor"},
    {INFORM_MAX,             					0,	"", "",                	                    ""}
...
int cwmp_event_init(cwmp_t *cwmp)
{
    ...
    {
    	cwmp_log_info("cwmp_event_init EVENT_REBOOT_NONE_FLAG");
	...
	if(cwmp->event_global.event_flag == EVENT_REBOOT_NONE_FLAG) {
		strcpy(cwmp->event_global.acs_url, cwmp->acs_url);
	}

	
    if (strcmp(cwmp->acs_url, cwmp->event_global.acs_url) != 0) {
	...
	if(cwmp->event_global.event_flag & EVENT_REBOOT_ACS_FLAG){
		//cwmp_event_set_value(cwmp, INFORM_BOOT, 1, NULL, 0, 0, 0);
        cwmp_event_set_value(cwmp, INFORM_MREBOOT, 1, cwmp->event_global.event_key, 0, 0, 0);
	}
...
int cwmp_event_set_value(cwmp_t *cwmp,  int event,   int value, const char * cmd_key, int fault_code, time_t start, time_t end)
{
	...
	// cwmp_log_debug( "cwmp_event_set_value begin, count=%d, max=%d", count, max);
		
    if(count  >= cwmp->el->size)
    {
     	cwmp_log_error( "event count=%d, max=%d\n", count, cwmp->el->size);
		return CWMP_ERROR;
    }

	// cwmp_log_debug( "cwmp_event_set_value pthread_mutex_lock");
	...
	// cwmp_log_debug( "cwmp_event_set_value k = %d", k);
	...
	// cwmp_log_debug( "cwmp_event_set_value fault_code = %d", fault_code);
...
int cwmp_event_clear_active_notsave(cwmp_t *cwmp)
{
    int     i;
