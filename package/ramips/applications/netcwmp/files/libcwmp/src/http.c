...
extern void parse_domain_name(char *host);
...
int http_parse_request(http_request_t * request, char *data, unsigned long len)
{
    ...
    if (data == NULL)
        return 0;
	if(!request) 
		cwmp_log_info("http_parse_request");
                ...
                char * response)
{
    ...
    tz_log_info("calc, user[%s], realm[%s], pwd[%s], nonce[%s], nc[%s], cnonce[%s], qop[%s], uri[%s]\r\n",
    	user, realm, pwd, nonce, nc, cnonce, qop, uri);

	tz_log_info("http_calc_digest_response,ha1=%s, ha2=%s, rsp=%s\r\n",
...
extern int base64_encode( unsigned char *in, unsigned char *out, int src_len );
    ...
    if(len2 > 0 || connection_request)
    {
        ...
        if((dest->auth.active == CWMP_FALSE) && (dest->auth_type == HTTP_DIGEST_AUTH))
        {
			...
			else
			{
				...
				base64_encode((unsigned char*)tmp, tmp_base64, strlen(tmp));
