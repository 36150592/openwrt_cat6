...
#ifdef COLOR_TIME_PRINT
    time_t now = time(NULL);
    struct tm *ptm = localtime(&now);
    fprintf(logger->file,
        GREEN"[%04d-%02d-%02dT%02d:%02d:%02d]"NONE,
        ptm->tm_year+1900,
        ptm->tm_mon+1,
        ptm->tm_mday,
        ptm->tm_hour,
...
void tz_log_info(const char * fmt, ...)
{
    va_list ap;
    if (g_ot_log_file_ptr->level < CWMP_LOG_INFO)
        return;
...
void _cwmp_log_write(int level, cwmp_log_t * log, const char * fmt, va_list ap)
{
    cwmp_log_t * logger;
...
void extz_log_info(const char * fmt, ...)
{
    va_list ap;
    if (g_ot_log_file_ptr->level < CWMP_LOG_INFO)
        return;
    ...
    va_start(ap, fmt);
    _cwmp_log_write(CWMP_LOG_INFO, g_ot_log_file_ptr, fmt, ap);
    va_end(ap);
}
