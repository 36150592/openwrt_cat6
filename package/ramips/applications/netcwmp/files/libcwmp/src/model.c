...
void walk_xmlDoc_attr_tree(XmlNode * param, int level)
{
	...
	extz_log_info("%s<>", fmt);
  	...
  	extz_log_info("%s=%s(%u) ", param->nodeName, param->nodeValue, (unsigned int)param->nodeValue);

  	XmlNode * next = param->nextSibling;
  	while(next)
	{
    	extz_log_info("%s=%s(%u) ", next->nodeName, next->nodeValue, (unsigned int)param->nodeValue);
...
int cwmp_model_create_parameter(parameter_node_t * param, xmlnode_t * node, model_func_t * func_list, int func_count,  pool_t * pool)
{
    ...
    // tz_log_info("create param_note_t, %s %s", node->nodeName, param->name);
...
int cwmp_model_init_object(cwmp_t * cwmp, parameter_node_t *param)
{ 
