...
qnode_t *data_in_queue(queue_t *q, void *data, int type)
{
	qnode_t *p;
	p = q->first;
	while(p != NULL) {
		if(p->data == data && p->datatype == type)
		{
			// cwmp_log_debug("data is already in queue");
			return p;
		}
		p = p->next;
	}
...
int queue_pop_with_arg(queue_t *q, void ** data, void **arg1, void **arg2) {
	if(q->first == NULL) { 
		cwmp_log_debug("queue is empty.");
		return -1;
	}
