...
static char const ident[]="$Id: route_processing.c 524 2015-09-19 12:56:27Z hb9xar $";
...
int route_preprocess(sip_ticket_t *ticket){
...

   {
	   ...
	   while( 1 )
	   {
		   route = (osip_route_t *) osip_list_get(&(mymsg->routes), index);
		   if ((route != NULL) && (route->url != NULL) &&
		      ...
		      if ((sts == STS_SUCCESS) &&
		         ...
		         osip_list_remove(&(mymsg->routes), index);
		         osip_route_free(route);
