...
static void get_wifi_ssid_suffix(int way, char* suffix)
{
	char* tmp_mac = NULL;
	if(way==1 || way==3) // mac-last-6
	{
		if(way == 1)
			tmp_mac = mac;
		else
		...
		char* last_six_of_mac=tmp_mac+6;
		strcpy(suffix, last_six_of_mac);
	}
	else if(way == 2 || way==4) //mac-last-3
	{
		if(way == 2)
			tmp_mac = mac;
		else
		...
		int ssid_suffix_value = strtol(tmp_mac+9, NULL, 16);
		ssid_suffix_value = ssid_suffix_value%1000;
		if(ssid_suffix_value < 10)
		{
			sprintf(suffix, "00%d",ssid_suffix_value);
...
static void get_24g_ssid(char* ssid)
{
	...
	if( tz_config_wifi_ssid_static_switch[0] != 'n' )
	{
		...
		if(strlen(tz_config_wifi_ssid) == 0)
		{
			...
			get_wifi_ssid_suffix(1, tz_config_wifi_ssid_suffix);
	...
	else
	{
		...
		get_wifi_ssid_suffix(suffix_way, tz_config_wifi_ssid_suffix);
...
static void get_random_wifi_pwd(int way, char* pwd)
{
	char* tmp_mac = NULL;
	
	if(way==1 || way == 3) // number, big zimu     SLT
	{
		if( way == 1 )
			tmp_mac = mac;
		else
		...
		m_mac[2] = HexToBin(tmp_mac+4);
		m_mac[3] = HexToBin(tmp_mac+6);
		m_mac[4] = HexToBin(tmp_mac+8);
		m_mac[5] = HexToBin(tmp_mac+10);
	...
	else if(way == 2 || way == 4) // number, big zimu little zimu     Dialog
	{
		unsigned char m_mac[6+1] = {0};
		int i = 0;
		int j = 1;
...
static void get_24g_pwd(char* pwd)
{
	...
	else
	{
		...
		get_random_wifi_pwd(random_way, tz_config_wifi_pwd);
	}
...
static void get_58g_ssid(char* ssid)
{
	char tz_config_wifi_ssid[56] = "";
	char tz_config_wifi_ssid_static_switch[56] = "";
	char tz_config_wifi_ssid_prefix[56] = "";
	...
	sprintf(shellcmd,"cat %s | grep %s= | awk -F '\"' '{print $2}'", FACTORY_CONFIG_FILE, "TZWIFI_58G_MAIN_WIFI_SSID_STATIC_SWITCH");
	read_memory(shellcmd, tz_config_wifi_ssid_static_switch, sizeof(tz_config_wifi_ssid_static_switch));
	util_strip_traling_spaces(tz_config_wifi_ssid_static_switch);

	if( tz_config_wifi_ssid_static_switch[0] != 'n' )
	{
		sprintf(shellcmd,"cat %s | grep %s= | awk -F '\"' '{print $2}'", FACTORY_CONFIG_FILE, "TZWIFI_58G_MAIN_WIFI_SSID");
		read_memory(shellcmd, tz_config_wifi_ssid, sizeof(tz_config_wifi_ssid));
		util_strip_traling_spaces(tz_config_wifi_ssid);

		if(strlen(tz_config_wifi_ssid) == 0)
		{
			strcpy(tz_config_wifi_ssid_prefix, "ZLT S21-5G-");
			get_wifi_ssid_suffix(1, tz_config_wifi_ssid_suffix);
			sprintf(tz_config_wifi_ssid,"%s%s",tz_config_wifi_ssid_prefix,tz_config_wifi_ssid_suffix);
		}
	}
	else
	{
		int suffix_way = 0;
		memset(shellcmd, 0, sizeof(shellcmd));
		sprintf(shellcmd,"cat %s | grep %s= | awk -F '\"' '{print $2}'", FACTORY_CONFIG_FILE, "TZWIFI_58G_MAIN_WIFI_SSID_PREFIX");
		read_memory(shellcmd, tz_config_wifi_ssid_prefix, sizeof(tz_config_wifi_ssid_prefix));
		util_strip_traling_r_n_t(tz_config_wifi_ssid_prefix);

		memset(shellcmd, 0, sizeof(shellcmd));
		sprintf(shellcmd,"cat %s | grep %s= | awk -F '\"' '{print $2}'", FACTORY_CONFIG_FILE, "TZWIFI_58G_MAIN_WIFI_SSID_SUFFIX");
		read_memory(shellcmd, tz_config_wifi_ssid_suffix, sizeof(tz_config_wifi_ssid_suffix));
		util_strip_traling_spaces(tz_config_wifi_ssid_suffix);

		if(tz_config_wifi_ssid_suffix[0] == '1')
			suffix_way = 1;  //2.4g mac last 6
		else if(tz_config_wifi_ssid_suffix[0] == '2')
			suffix_way = 2; //2.4g mac last 3
		else if(tz_config_wifi_ssid_suffix[0] == '3')
			suffix_way = 3; //5.8g mac last 3
		else if(tz_config_wifi_ssid_suffix[0] == '4')
			suffix_way = 4; //5.8g mac last 3
		else
			suffix_way = 1;
...
static void get_58g_pwd(char* pwd)
{
	char tz_config_wifi_pwd[56] = "";
	char tz_config_wifi_pwd_random[56] = "";
	char tz_config_wifi_pwd_random_way[56] = "";
	...
	memset(shellcmd, 0, sizeof(shellcmd));
	sprintf(shellcmd,"cat %s | grep %s= | awk -F '\"' '{print $2}'", FACTORY_CONFIG_FILE, "TZWIFI_58G_MAIN_WIFI_PWD_RANDOM");
	read_memory(shellcmd, tz_config_wifi_pwd_random, sizeof(tz_config_wifi_pwd_random));
	util_strip_traling_spaces(tz_config_wifi_pwd_random);
	...
	{
		memset(shellcmd, 0, sizeof(shellcmd));
		sprintf(shellcmd,"cat %s | grep %s= | awk -F '\"' '{print $2}'", FACTORY_CONFIG_FILE, "TZWIFI_58G_MAIN_WIFI_PWD");
		read_memory(shellcmd, tz_config_wifi_pwd, sizeof(tz_config_wifi_pwd));
		util_strip_traling_spaces(tz_config_wifi_pwd);
	...
	else
	{
		int random_way = 0;
		memset(shellcmd, 0, sizeof(shellcmd));
		sprintf(shellcmd,"cat %s | grep %s= | awk -F '\"' '{print $2}'", FACTORY_CONFIG_FILE, "TZWIFI_58G_MAIN_WIFI_PWD_RANDOM_WAY");
		read_memory(shellcmd, tz_config_wifi_pwd_random_way, sizeof(tz_config_wifi_pwd_random_way));
		util_strip_traling_spaces(tz_config_wifi_pwd_random_way);
		...
		if(tz_config_wifi_pwd_random_way[0] == '1')
			...
			random_way = 2;
		else if(tz_config_wifi_pwd_random_way[0] == '3')
			random_way = 3;
		else if(tz_config_wifi_pwd_random_way[0] == '4')
			random_way = 4;
		else
			random_way = 2;
...
void form_wifi_58g_ssid_pwd_to_file(char* fileName)
{
	char ssid[64] = "";
	char pwd[64] = "";
	char shellcmd[256] = "";
	...
	memset(shellcmd, 0, sizeof(shellcmd));
	sprintf(shellcmd, "echo wifi_58g_ssid=\"%s\" > %s", ssid, fileName);
	system(shellcmd);

	memset(shellcmd, 0, sizeof(shellcmd));
	sprintf(shellcmd, "echo wifi_58g_pwd=\"%s\" >> %s", pwd, fileName);
	system(shellcmd);
}
