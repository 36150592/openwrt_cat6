...
int parse_qmi(PUCHAR qmi, ULONG TotalDataLength) 
{
    ...
    if ((qmi_hdr->QMIType == QMUX_TYPE_CTL) ) {
        ...
        printf("QMICTLType:         %04x\t%s\n", qmictl_msg_hdr->QMICTLType,
        QMUX_NAME(qmux_ctl_QMICTLType, qmictl_msg_hdr->QMICTLType, tag));     
        printf("Length:             %04x\n", qmictl_msg_hdr->Length);
    ...
    else
    {
        ...
        switch (qmi_hdr->QMIType) {
            case QMUX_TYPE_DMS:
                printf("Type:               %04x\t%s\n", Message->QMUXMsgHdr.Type,
                QMUX_NAME(qmux_dms_Type, Message->QMUXMsgHdr.Type, tag));
            break;
            case QMUX_TYPE_NAS:
                printf("Type:               %04x\t%s\n", Message->QMUXMsgHdr.Type,
                QMUX_NAME(qmux_nas_Type, Message->QMUXMsgHdr.Type, tag));
            break;
            case QMUX_TYPE_WDS:
                printf("Type:               %04x\t%s\n", Message->QMUXMsgHdr.Type,
                QMUX_NAME(qmux_wds_Type, Message->QMUXMsgHdr.Type, tag));
            break;
            case QMUX_TYPE_WMS:
                printf("Type:               %04x\t%s\n", Message->QMUXMsgHdr.Type,
                QMUX_NAME(qmux_wms_Type, Message->QMUXMsgHdr.Type, tag));
            break;
            case QMUX_TYPE_WDS_ADMIN:
                printf("Type:               %04x\t%s\n", Message->QMUXMsgHdr.Type,
                QMUX_NAME(qmux_wds_admin_Type, Message->QMUXMsgHdr.Type, tag));
            break;
            case QMUX_TYPE_PDS:
            case QMUX_TYPE_QOS:
            case QMUX_TYPE_CTL:                
            default:
                printf("Type:               %04x\t%s\n", Message->QMUXMsgHdr.Type, "unkonw!");
            break;    
        }
        printf("Length:             %04x\n", Message->QMUXMsgHdr.Length);
    }
