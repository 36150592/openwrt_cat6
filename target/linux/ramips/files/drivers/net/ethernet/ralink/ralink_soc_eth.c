...
static int fe_open(struct net_device *dev)
{
	...
	netif_start_queue(dev);
        ...
        if(priv->mii_bus)
        {
        priv->mii_bus->write(priv->mii_bus,0,0,0x3300);
        priv->mii_bus->write(priv->mii_bus,1,0,0x3300);
        priv->mii_bus->write(priv->mii_bus,2,0,0x3300);
        priv->mii_bus->write(priv->mii_bus,3,0,0x3300);
        }
        else{
        printk("priv->mii_bus is NULL\n");
        }
